...
#microserv = ['adservice', 'cartservice']
gitrepo = 0
env = "dev"
scanloglocation = "/home/ubuntu/" + env + "/scanlogs"
...
for serv in range(len(microserv)):
    summary = []
    print('##########################################################')
    print('Checking %s' % microserv[serv])
    print('##########################################################\n')
    ...
    if(servstat == 1):
        print("Checking tags for %s in git in repo\n" % microserv[serv])
        if (gitrepo ==1):
            print("Not possible\n")
        else:
            command1 = 'skopeo list-tags --creds cmupro7:grp5pro7cmu@1234 docker://cmupro7/' + microserv[serv]
            ...
            for line in range(len(output1)):
                if re.match(r'^\s+\"\d+\"(\,)?', output1[line], re.M | re.I):
                    tagsko = re.search(r'^\s+\"(\d+)\"(\,)?', output1[line], re.M | re.I)
                    taggitlist.append(int(tagsko.group(1)))
            taggit = max(taggitlist)
            print("The latest tag in git for %s is %d\n" %(microserv[serv], taggit))
        scanphase = 0
        if(taggit > int(tagprod.group(1))):
            print("Tag %d in git is greater than tag %s in production, Moving to scan phase\n" % (taggit, tagprod.group(1)))
            summary = [microserv[serv],'needscan']
            scanphase = 1
        elif(taggit == int(tagprod.group(1))):
            print("Tag %d in git is equal to tag %s in production, Moving to next service\n" % (taggit, tagprod.group(1)))
            summary = [microserv[serv],'equaltag']
        elif(taggit < int(tagprod.group(1))):
            print("Tag %d in git is less than tag %s in production, Need to check what went wrong\n" % (taggit, tagprod.group(1)))
            summary = [microserv[serv],'impropertag']
        if(scanphase == 1):
